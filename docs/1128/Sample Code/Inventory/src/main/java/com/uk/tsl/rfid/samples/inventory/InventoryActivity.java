...
import com.uk.tsl.rfid.asciiprotocol.commands.SystemCommand;
import com.uk.tsl.rfid.asciiprotocol.device.ConnectionState;
import com.uk.tsl.rfid.asciiprotocol.device.IAsciiTransport;
import com.uk.tsl.rfid.asciiprotocol.device.IConfigurableAsciiTransport;
...
import com.uk.tsl.rfid.samples.inventory.databinding.ActivityInventoryBinding;
...
//            editor.putString(getString(R.string.last_connected_reader), getCommander().getReader().getSerialNumber());
//            editor.apply();

            // Only need to optimise comms if using the ePop-Loqconnector as it defaults to 115200 for backwards compatibility.
            if( mReader.getActiveTransport() instanceof IConfigurableAsciiTransport)
            {
                //int defaultCommSpeed = 115200;
                int commSpeed = 1200000;
                //int commSpeed = 1500000;
                if (!optimiseComms(mReader, commSpeed)) {mReader = null;}
            }
        }
        else if( commanderConnectionState == ConnectionState.LOST)
        {
            if( mReader != null )
            {
                Log.d(getClass().getName(), String.format("Reader: %s lost !!!", mReader.getDisplayName()));
            }
            // Lost connection - choose reader again
            mReader = null;
        }
    };

    /**
     * Ensure that the comms (via ePop-Loq) are running as fast as possible
     * This requires a Reader with ASCII Protocol 3.0 or greater
     * Reader may be disconnected if error occurs
     *
     * @param reader the reader in use
     * @param commSpeed the desired comm speed (115200 - 1500000)
     * @return true if reader is connected
     */
    private boolean optimiseComms(Reader reader, int commSpeed)
    {
        boolean isConnected = true;
        if( reader != null )
        {
            // If the connection is USB then see if the connection can be made faster
            IConfigurableAsciiTransport transport = (IConfigurableAsciiTransport)reader.getActiveTransport();
            if (transport.type() == TransportType.USB)
            {
                // Check to see if the reader supports fast connections
                if (reader.getDeviceProperties().getInformationCommand().getAsciiProtocol().startsWith("3"))
                {
                    try
                    {
                        Log.d("INV", String.format("Changing comm speed..."));
                        SystemCommand sCommand = SystemCommand.synchronousCommand();
                        sCommand.setEPopLoqBaudRate(commSpeed);
                        getCommander().executeCommand(sCommand);
                        if( sCommand.isSuccessful() )
                        {
                            // Reader has changed baud rate so adjust transport to match
                            if (transport.setBaudRate(commSpeed))
                            {
                                Log.d("INV", String.format("Baud rate: " + commSpeed));
                            }
                            else
                            {
                                // Unable to change transport
                                reader.disconnect();
                                isConnected = false;
                                Log.d("INV", String.format("!!! Unable to change Baud rate - disconnecting... !!!"));
                                Log.d("INV", String.format("!!! Physically disconnect the ePop-Loq device. !!!"));
                            }
                        }
                        else
                        {
                            Log.d("INV", String.format("!!! Unsupported Baud rate (%d) !!!", commSpeed));
                        }
                    }
                    catch(Exception e)
                    {
                        Log.d("INV", String.format("!!! Unsupported Baud rate - ignoring... !!!"));
                    }
                }
            }
        }
