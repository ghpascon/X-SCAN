...
public class TagFinderActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        mTargetTagEditText.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            @Override
...
//                final String value = level == null ? "---" : String.format("%d %%", mPercentageConverter.asPercentage(level));
                final String value = level == null ? "---" : String.format("%d %% (%d)", mPercentageConverter.asPercentage(level), level);
    ...
    private void UpdateUI()
    {
        ...
        if( isConnected)
        {
            ...
            {
                instructions = getResources().getString(R.string.rssi_instruction_placeholder_text);
            }
            else
            {
