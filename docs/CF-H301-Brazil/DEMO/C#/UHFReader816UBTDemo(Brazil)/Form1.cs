    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
           ...
           // SetButtonBold(btDisConnect232);
            ...
            for (i = 0; i < 256; i++)
            {
                comboBox1.Items.Add(i.ToString() + "*10s");
            }
            comboBox1.SelectedIndex = 12;

            for (i = 1; i < 256; i++)
            {
                comboBox2.Items.Add(i.ToString() + "*1s");
            }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            byte FreeTime = 0;
            byte TrigReadTime = 0;
            FreeTime = Convert.ToByte(comboBox1.SelectedIndex);
            TrigReadTime = Convert.ToByte(comboBox2.SelectedIndex + 1);
            fCmdRet = RWDev.SetWorkTime(ref fComAdr, FreeTime, TrigReadTime, frmcomportindex);
            if (fCmdRet != 0)
            {
                string strLog = "";
                strLog = "Set failed: " + GetReturnCodeDesc(fCmdRet);
                WriteLog(lrtxtLog, strLog, 1);
            }
            else
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            byte FreeTime = 0;
            byte TrigReadTime = 0;
            fCmdRet = RWDev.GetWorkTime(ref fComAdr, ref FreeTime, ref TrigReadTime, frmcomportindex);
            if (fCmdRet != 0)
            {
                string strLog = "";
                strLog = "Get failed: " + GetReturnCodeDesc(fCmdRet);
                WriteLog(lrtxtLog, strLog, 1);
            }
            else
            {
                comboBox1.SelectedIndex = FreeTime;
                comboBox2.SelectedIndex = TrigReadTime - 1;
        ...
        private void radioButton_band12_CheckedChanged(object sender, EventArgs e)
        {
            ...
            ComboBox_dmaxfre.SelectedIndex = 34;
            ComboBox_dminfre.SelectedIndex = 0;
        }
