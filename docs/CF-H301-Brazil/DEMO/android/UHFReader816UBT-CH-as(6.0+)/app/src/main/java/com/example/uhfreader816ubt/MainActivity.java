...
public class MainActivity extends AppCompatActivity implements OnClickListener{
    ...
    public static final String EXTRA_MODE = "EPCC1-G2 TEST";
		...
		lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0,  View v, int position, long id) {
		        ...
		        if (MyService.mScanning){
		        	 stopdiscover=false;
					 scanLeDevice(false);
					 scan_btn.setText(getString(R.string.btrSearch));
					 scan_flag=true;
		        }
		...
		if (Build.VERSION.SDK_INT >= 23) {
			...
			if(checkCallPhonePermission != PackageManager.PERMISSION_GRANTED){
				...
				if(ActivityCompat.shouldShowRequestPermissionRationale(this, android.Manifest.permission.ACCESS_COARSE_LOCATION))
					Toast.makeText(this,"BLE Need LOCATION", Toast.LENGTH_LONG).show();//ACCESS_COARSE_LOCATION
	...
	public void onClick(View view) {
		...
		if(view ==scan_btn)
		{
			 if(scan_flag)
			 {
				 ...
				 lv.setAdapter(mleDeviceListAdapter);
				 scanLeDevice(true);
				 scan_btn.setText(getString(R.string.btrstop));
				 Thread thread=new Thread(new Runnable()  
		            {  
		                @Override  
		                public void run()  
