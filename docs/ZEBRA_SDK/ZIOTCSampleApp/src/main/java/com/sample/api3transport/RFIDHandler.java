...
import com.sample.api3transport.ui.Connect.ConnectFragment;
...
public class RFIDHandler {
    ...
    public static RFIDReader mRfidReader;
...
“Antenna Power Level Adjusted” information event is sent to host if antenna power level is higher than 24dbm when reader is placed in cradle
       ...
       // reader.Events.setInfoEvent(true);
        ...
        try {
            ...
            mReader.Config.getDeviceStatus(false,false,false,true,false);
        } catch (InvalidUsageException e) {
            //e.printStackTrace();
        } catch (OperationFailureException e) {
            //e.printStackTrace();
...
//                    new AsyncDataUpdate().execute(myTags);
        ...
        public void eventStatusNotify(RfidStatusEvents rfidStatusEvents) {
            ...
            if(rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.CONNECTED_INTERFACE_EVENT){
                String data = rfidStatusEvents.StatusEventData.connectedInterfaceEventData.getInterface();
                Log.d(TAG,"ConnectedInterface rfid getInterface : "+data );
            }

            if(rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.DEBUG_INFO_EVENT){
                ...
                Log.d(TAG,"debugInfoData rfid level : "+level+" message:"+message);
            }
